<template>
    <v-card class="mx-auto" max-width="368">
        <v-card-item title="Florida">
            <template #subtitle>
                <v-icon icon="i-mdi-alert" size="18" color="error" class="me-1 pb-1"></v-icon>
                Extreme Weather Alert
            </template>
        </v-card-item>
        <v-card-text class="py-0">
            <v-row align="center" no-gutters>
                <v-col class="text-h2" cols="6">54&deg;F</v-col>
                <v-col cols="6" class="text-right">
                    <v-icon color="error" icon="i-mdi-weather-hurricane" size="88"></v-icon>
                </v-col>
            </v-row>
        </v-card-text>
        ...
        <v-expand-transition>
            <div v-if="expand">
                ...
                <v-list class="bg-transparent">
                    <v-list-item v-for="item in forecast" :key="item.day" :title="item.day" :append-icon="item.icon" :subtitle="item.temp">
</v-list-item>
                </v-list>
            </div>
        </v-expand-transition>
